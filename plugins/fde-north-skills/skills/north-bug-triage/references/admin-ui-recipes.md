...
Copy-paste playwright-cli recipes for the North admin app. Mount path locally is `http://localhost:5001/admin/`; on staging it is `https://stg.demo.cloud.cohere.com/admin/`. For end-user flows, see `north-ui-recipes.md`. For tool-level quirks, see `playwright-cli-gotchas.md`. For auth/data strategy, see `repro-strategy.md`.
...
If the static connector button isn't visible, the admin is only offering real SSO connectors — in staging, ask the user to complete the SSO flow in the Playwright window (`AskQuestion`).
...
1. **Create a fresh throwaway user via the Create User recipe above, then log in as them in the Northa UI**. Fastest, keeps roles clean.
...
If the video only shows the login/connector-selection page, delete it — see the partial-video rule in `repro-strategy.md`.
