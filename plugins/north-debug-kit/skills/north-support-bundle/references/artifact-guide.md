...
Reference content for the `north-support-bundle` skill's workflow steps. Use this file after the caller reaches the artifact-analysis step:

- When invoked from `north-bug-triage`, that is step 4 (after Explore and Reproduce).
- When invoked standalone (customer dropped a bundle), that is step 3 (after identification and ambiguity resolution).

## Recognizing a support bundle

When called from `north-bug-triage`, the input for this decision is a single file: `.triage-artifacts-{issue_id}/linear-context.md`, produced by the triage's Explore step. Scan the **Artifact Index** table — each row has `Title / filename | URL | Source`.

When called standalone, the input is either a direct URL / local path, or the list produced by `linear-cli` against a user-supplied issue id.

Classify a row (or a standalone filename) as a *candidate support bundle* if **any** of the following match (all case-insensitive):
...
- **Exactly one candidate**: proceed automatically. Record the bundle URL and Source in the caller's report (e.g. `REPORT.md` → *Evidence*) so the choice is auditable.
- **Zero or multiple candidates**: do not guess. Use `AskQuestion` with the structured options in the next section.
- **One candidate, but its title/URL strongly disagrees with the incident timestamp or customer** (when the caller provides metadata): treat as ambiguous and fall through to the multi-candidate branch.
...
If the user chooses "proceed without", explicitly note the gap in the caller's report under *Missing evidence* so downstream agents know the analysis was run without a bundle.
...
Do not guess which bundle is relevant. The wrong bundle can waste significant analysis time.
