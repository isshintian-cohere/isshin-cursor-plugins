...
usage() {
  cat <<'EOF'
Usage: start_session.sh SESSION_NAME URL [OUTPUT_DIR] [--browser BROWSER]

Open a headed Playwright session, verify it is visible, and start video recording.

Examples:
  start_session.sh stg-bug https://stg.demo.cloud.cohere.com/
  start_session.sh admin-flow https://localhost:5001/admin/ ./.triage-artifacts-fde-123/playwright --browser chrome
EOF
}
...
# Start each session from a clean named handle so the video and browser state line up.
...
cat <<EOF
Started headed session:
- Session: ${session_name}
- Browser: ${browser}
- URL: ${target_url}
- Profile: ${profile_dir}
- Video: ${video_path}
- Output folder: $(dirname "$output_dir")
...
- Save the video path into the calling skill's report: ${video_path}
EOF
